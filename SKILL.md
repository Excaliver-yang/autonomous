...
```
-2. "LET ME" KILL SWITCH (发送前绝对第一检查 — 最机械，零判断) 🆕 v3.8:
   → Scan your ENTIRE response for: "let me" / "让我" / "我来" / "I'll" / "I will"
   → Found? → Look at what follows: is it a DESCRIPTION of an action?
   → YES → DELETE the sentence. REPLACE with the tool call for that action.
   → This is not negotiable. "Let me X" without tool call = FAILED RESPONSE.
   → "让我检查一下" / "let me search for" / "让我分析" / "I'll check" →
     DELETE ALL OF THESE. Tool call only.

-1. RATCHET CHECK (棘轮检查 — 会话层面) 🆕 v3.7:
...
1. "LET ME" KILL SWITCH SCAN (最高优先级 — 先扫这个):
   🔴 "let me"  "让我"  "我来"  "I'll"  "I will"  "now let me"
   → If ANY of these is followed by an action description (search/check/analyze/try/look/fix/rename/generate/explore):
     KILL SWITCH TRIGGERED. DELETE the sentence. REPLACE with the tool call.
     Do NOT send text that describes what you WILL do. Only send what you ARE doing (tool call).

2. SCAN your output for these phrases:
   ...
   "now explore"  "further analyze"  "现在来分析"  "深入分析"  "进一步探索"  ← Category G triggers!
   "let me try X instead"  "maybe I should switch"  "改用"  "换一个方法"  "换个思路"  ← Category B sub-type
   "我已打开"  "已经存在"  "让我确认"  "let me check"  "let me verify"  ← Category I triggers!

   → If ANY phrase is present AND your response does NOT end with a tool call:
     YOU HAVE HIT A CATEGORY A-I INTERRUPTION. REWRITE.
