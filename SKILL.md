...
description: 'Autonomous Continuity Engine v3.7 — prevents ALL execution interruption in multi-step tasks. 9-category interruption classification (A: Plan Statement / B: Error Diagnosis / C: Progress Report / D: Observation-Conclusion / E: Thinking Process / F: Acknowledgement / G: Exploration Chain Break / H: Ratchet Continuity / I: User Context Neglect) with mechanical Colon Rule, completion hallucination detection, ratchet level escalation (total-push tracking, not just same-category), silent method switching, command syntax loop prevention, user context leverage, mid-task self-improvement, and project auto-configuration files. MODEL-AGNOSTIC: all models, all providers, all proxy configurations. Zero "继续" required.'
---

# Autonomous Continuity Engine v3.7
...
Level 3 — 重度棘轮 (Heavy Ratchet) — 用户推进 3+ 次:
  ...
  → 不因为以下原因而停止:
    · "报告发现/分析结论" — 继续链入下一步
    · "问询用户意见/确认" — 自主决策
    · 任何 Category A-H 的单个模式 — 这些不再是有效停止理由
    · "需要进一步思考" — 思考然后立即执行
